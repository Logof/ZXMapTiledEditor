...
import tiled.mapeditor.selection.*;
 ...
 * @version $Id$
 */
public abstract class ZXMapView extends JPanel implements Scrollable {
    private Logger log = LoggerFactory.getLogger(ZXMapView.class);
    ...
    public static final int PF_ZX_SCREEN_COORDINATES = 0x10;

    /**
     * The default grid color (black).
     */
    public static final Color DEFAULT_GRID_COLOR = Color.black;
    public static final Color DEFAULT_ZX_SCREEN_GRID_COLOR = Color.green;
    ...
    protected boolean showZXScreenGrid;
    protected boolean antialiasGrid;
    protected Color gridColor;
    protected Color gridZXScreenColor;
     ...
     */
    protected ZXMapView(Map map) {
        // Setup static bits on first invocation
        if (ZXMapView.propertyFlagImage == null) {
            try {
                ZXMapView.propertyFlagImage = Resources.getImage("propertyflag-12.png");
            } catch (Exception e) {
            }
        }
     ...
     */
    public static ZXMapView createViewforMap(Map p) {
        ZXMapView mapView = null;
        int orientation = p.getOrientation();
        if (orientation == Map.MDO_ORTHO) {
            mapView = new OrthoZXMapView(p);
        }
        return mapView;
    }
    ...
    private Rectangle pixelToScreenCoords(MapLayer layer, Rectangle rect) {
        ...
        Rectangle rectangle = new Rectangle(p0.x, p0.y, p1.x - p0.x, p1.y - p0.y);
        return rectangle;
    }
    ...
    public void setZXScreenGridColor(Color gridColor) {
        this.gridZXScreenColor = gridColor;
        repaint();
    }
    ...
    public boolean getShowZXScreenGrid() {
        return showZXScreenGrid;
    }
    ...
    public void setShowZXScreenGrid(boolean showZXScreenGrid) {
        this.showZXScreenGrid = showZXScreenGrid;
        revalidate();
        repaint();
    }
    ...
    public void paintComponent(Graphics g) {
        ...
        if (showZXScreenGrid) {
            g2d.setColor(gridZXScreenColor);
            // Grid opacity
            if (gridOpacity < 255) {
                g2d.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_ATOP, (float) gridOpacity / 255.0f));
            } else {
                g2d.setComposite(AlphaComposite.SrcOver);
            }
    ...
    /**
     * Draws a mesh for this layer to fit the ZX Spectrum game screen.
     *
     ...
     *
     * @param layer layer to calculate the coordinates for or null if
    ...
    public void setCurrentLayer(MapLayer layer) {
        ...
        if (getMode(PF_COORDINATES) || getMode(PF_ZX_SCREEN_COORDINATES) || getShowGrid() || getShowZXScreenGrid()) {
            repaint();
        }
    }
